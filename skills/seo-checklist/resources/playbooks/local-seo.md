<!--
Bundled playbook. Adapted from the `rank-local` skill, written by the owner of
this plugin — copied in deliberately so the audit does not depend on a separate
skill being installed on whatever machine it runs on.
-->

# Local SEO playbook

Answers the checklist's local block: **LO-196** (does this business actually
depend on local traffic) and **LO-199** (Business Profile set up and optimised),
plus the NAP and citation ground the local profile assumes.

**This is a playbook, not a data source.** It cannot read the live Business
Profile, so LO-199 stays `MANUAL` — a person still has to open GBP and check.
What it changes is that the person gets concrete steps instead of an item title.
Never mark LO-199 `PASS` from this file alone.

Run it when the site profile is `local`, or when the `seo-llm-market` agent
answers LO-196 `PASS`.
